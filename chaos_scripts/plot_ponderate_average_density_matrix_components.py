...
for dir in dir_per:
    ...
    a=1
    for key in keys1:
        alldata_per[a].append(np.sum(np.multiply(np.array(hf.get(key)),np.array(hf.get(keys3[a-1]))))/np.sum(np.array(hf.get(keys3[a-1]))))
        a=a+1

    b=7
    for key in keys2:
        alldata_per[b].append(np.sum(np.multiply(np.sqrt(np.array(hf.get(key[0]))**2+np.array(hf.get(key[1]))**2),np.array(hf.get(keys3[b-7]))))/np.sum(np.array(hf.get(keys3[b-7]))))
        b=b+1
...
for dir in dir_ori:
    ...
    a=1
    for key in keys1:
        alldata_ori[a].append( np.sum(np.multiply(np.array(hf.get(key)),np.array(hf.get(keys3[a-1]))))/np.sum(np.array(hf.get(keys3[a-1]))) )
        a=a+1

    b=7
    for key in keys2:
        alldata_ori[b].append( np.sum(np.multiply(np.sqrt(np.array(hf.get(key[0]))**2+np.array(hf.get(key[1]))**2),np.array(hf.get(keys3[b-7]))))/np.sum(np.array(hf.get(keys3[b-7])))  )
        b=b+1
...
for i in range(1,len(alldata_per)):
    ...
    plt.xlabel(r'Time (s)') 
    plt.legend()
    plt.savefig('plots/pon_'+names[i-1])  
    plt.clf() 
