...
filename_bang = "/global/project/projectdirs/m3761/FLASH/FFI_3D/NSM/sim1/from_payne/raw/reduced_data_NSM_sim.h5"

t,Nee = plotdata(filename_emu_2f_lowres,0,0)
t,Nxx = plotdata(filename_emu_2f_lowres,1,1)
axes[0].plot(t, Nee * n_2F/n_tot, 'k-', alpha=0.5)
t,Nee = plotdata(filename_emu_2f,0,0)
t,Nxx = plotdata(filename_emu_2f,1,1)
axes[0].plot(t, Nee * n_2F/n_tot, 'k-', label=r'${\rm Emu\,\,(2f)}$')

t,Nee = plotdata(filename_emu_3f_lowres,0,0)
t,Nxx = plotdata(filename_emu_3f_lowres,1,1)
axes[0].plot(t, (Nee), 'k--', alpha=0.5)
t,N = plotdata(filename_emu_3f,0,0)
axes[0].plot(t, N, 'k--', label=r'${\rm Emu\,\,(3f)}$')

t,Nee = plotdata(filename_bang,0,0)
t,Nxx = plotdata(filename_bang,1,1)
axes[0].plot(t, Nee * n_2F/n_tot, 'r-', label=r'${\rm FLASH\,\,(2f)}$')

t,N = plotdata(filename_emu_2f_lowres,0,1)
axes[1].semilogy(t, N * n_2F/n_tot, 'k-', alpha=0.5)
t,N = plotdata(filename_emu_2f,0,1)
axes[1].semilogy(t, N * n_2F/n_tot, 'k-', label=r'${\rm Emu\,\,(2f)}$')

t,N = plotdata(filename_emu_3f_lowres,0,1)
axes[1].semilogy(t, N, 'k--', alpha=0.5)
t,N = plotdata(filename_emu_3f,0,1)
axes[1].semilogy(t, N, 'k--', label=r'${\rm Emu\,\,(3f)}$')

t,N = plotdata(filename_bang,0,1)
axes[1].semilogy(t, N * n_2F/n_tot, 'r-', label=r'${\rm FLASH\,\,(2f)}$')
